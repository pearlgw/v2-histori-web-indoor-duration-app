  ...
  async function onSubmit(values: z.infer<typeof RegisterSchema>) {
    try {
      ...
      if (response?.meta?.status_code === 201) {
        toast.success(
          `Asisten '${response.data.fullname}' berhasil ditambahkan`
        );
        form.reset();
        setPreview(null);
      } else {
        toast.error(
          response.meta.message_error || "Gagal menambahkan asisten."
        );
      }
    } catch (err: any) {
      ...
      >
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input className="h-12" placeholder="Fullname" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <div className="flex flex-col gap-1 text-gray-500">
          ...
          <p className="text-xs italic">
            1. Foto yang diupload harus jelas dan memiliki pencahayaan yang
            baik.
          </p>
          <p className="text-xs italic">
            2. Foto berukuran tidak lebih dari 5 mb!
          </p>
          <p className="text-xs italic">
            3. Format photo yang harus diupload adalah JPG, PNG, JPEG
          </p>
